...
// Declare svg as a window property instead of a local variable
window.svg = null;
...
function StateTempω(maxTemp, maxω, name, totalPressure) {
    var self = this;
    self.temperature = ko.observable(minTemp);
    self.humidityRatio = ko.observable(0);
    self.pv = ko.computed(() => pvFromw(self.humidityRatio(), totalPressure));
    self.name = ko.observable(name);
    
    // Add computed values for debugging
    self.temperatureC = ko.computed(() => fahrenheitToCelsius(self.temperature()));
    self.relativeHumidity = ko.computed(() => {
        const pv = pvFromw(self.humidityRatio(), totalPressure);
        const satPv = satPressFromTempIp(self.temperature());
        return (pv / satPv) * 100;
    });
}
...
function ViewModel() {
    ...
    self.xAxis = ko.computed(() => {
        return d3
            ...
            .tickFormat(temp => fahrenheitToCelsius(temp).toFixed(1));
    });
    ...
    ko.computed(() => {
        ...
        selection.enter()
            .append("text")
            .text(d => fahrenheitToCelsius(d).toFixed(1))
            .attr("dx", "-13")
            .attr("font-size", "10px")
            .merge(selection)
    ...
    ko.computed(() => {
        var selection = d3.select("#state-points").selectAll("circle")
            .data(self.states());
            
        // Enter new points with larger radius and stroke
        selection.enter()
            .append("circle")
            .attr("r", 6) // Increased from 5
            .style("fill", "red")
            .style("stroke", "white") // Add white stroke
            .style("stroke-width", "2")
            .merge(selection)
            .attr("cx", d => self.xScale()(d.temperature()))
            .attr("cy", d => self.yScale()(d.humidityRatio()));
        ...
        const state = self.states()[0];
        if (state) {
            console.log('State point coordinates:', {
                temp: state.temperature(),
                humidity: state.humidityRatio(),
                x: self.xScale()(state.temperature()),
                y: self.yScale()(state.humidityRatio()),
                viewBox: d3.select("#chartsvg").attr("viewBox")
            });
        }
    });

    // Update these values to match your chart's requirements
    self.maxTemp = ko.observable(120); // Maximum temperature in Fahrenheit
    self.maxHumidityRatio = ko.observable(0.03); // Maximum humidity ratio
    ...
    self.statePoints = window.svg.append("g")
        .attr("id", "state-points");

    ko.computed(() => {
        var selection = self.statePoints.selectAll("circle")
            .data(self.states());
        ...
        if (state) {
            console.log('State point coordinates:', {
                ...
                y: self.yScale()(state.humidityRatio()),
                viewBox: d3.select("#chartsvg").attr("viewBox"),
                maxHumidity: self.maxHumidityRatio(),
                scale: {
                    xDomain: self.xScale().domain(),
                    yDomain: self.yScale().domain(),
                    xRange: self.xScale().range(),
